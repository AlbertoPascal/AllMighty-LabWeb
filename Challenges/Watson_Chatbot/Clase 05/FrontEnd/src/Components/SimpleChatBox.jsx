...
class Review extends React.Component {
  ...
  async componentWillMount() {
    ...
    console.log(this.call_whatson(msg.value));
     ...
     this.setState({inte: await this.call_whatson_intent(msg.value), msg:  await this.call_whatson(msg.value)});
  }
  ...
  async call_whatson(sent_msg){
    return await axios.post('http://localhost:5002/getMessage', { message: sent_msg} )
        .then(resp => {
          ...
          return resp.data.este_es_el_mensaje;
        }
  ...
  render() {
    ...
    {
      return (
        <Carousel class = "container" width = "100%">{ReactHtmlParser(msg_td)}</Carousel>
      );
    }
    return (
      <div style={{ width: '100%' }}>
